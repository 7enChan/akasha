#!/usr/bin/env bash
#
# Build akasha binaries for all platforms locally.
...
#     akasha-darwin-arm64.tar.gz
#     akasha-darwin-x64.tar.gz
#     akasha-linux-x64.tar.gz
#     akasha-linux-arm64.tar.gz
#     akasha-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/akasha.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/akasha
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating akasha-$platform.zip..."
        (cd $platform && zip -r ../akasha-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating akasha-$platform.tar.gz..."
        mv $platform akasha && tar -czf akasha-$platform.tar.gz akasha && mv akasha $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../akasha-$platform.zip)
    else
        tar -xzf akasha-$platform.tar.gz && mv akasha $platform
    fi
done
