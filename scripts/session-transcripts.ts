 ...
 *   --analyze      Spawn akasha subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("akasha", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn akasha: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".akasha/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn akasha subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".akasha/agent/AGENTS.md");
