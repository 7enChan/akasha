...
const agentDirEnvName = "AKASHA_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "AKASHA_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific AKASHA_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force AKASHA_OFFLINE=1 / AKASHA_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.AKASHA_OFFLINE = "1";
		env.AKASHA_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "akasha-startup-benchmark-")) : undefined;
