...
describe.skipIf(!oauthToken)("Anthropic OAuth tool name normalization", () => {
	...
	it("should handle akasha's built-in tools (read, write, edit, bash)", async () => {
		// Akasha's tools use lowercase names, CC uses PascalCase
		const readTool: Tool = {
			name: "read",
	...
	it("should NOT map find to Glob - find is not a CC tool name", async () => {
		// Akasha has a "find" tool, CC has "Glob" - these are DIFFERENT tools
