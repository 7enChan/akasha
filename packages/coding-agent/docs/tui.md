> akasha can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@earendil-works/akasha-tui`](https://github.com/7enChan/akasha/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@earendil-works/akasha-tui";
...
```typescript
import { Container, type Focusable, Input } from "@earendil-works/akasha-tui";
...
**In custom tools** via `akasha.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = akasha.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@earendil-works/akasha-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@earendil-works/akasha-tui";
```
...
```typescript
import { matchesKey, Key } from "@earendil-works/akasha-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@earendil-works/akasha-tui";
...
} from "@earendil-works/akasha-tui";
...
```typescript
import { getMarkdownTheme } from "@earendil-works/akasha-coding-agent";
import { Markdown } from "@earendil-works/akasha-tui";
...
Set `AKASHA_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
AKASHA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@earendil-works/akasha-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@earendil-works/akasha-coding-agent";
import { DynamicBorder } from "@earendil-works/akasha-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@earendil-works/akasha-tui";
...
```typescript
import { BorderedLoader } from "@earendil-works/akasha-coding-agent";

akasha.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@earendil-works/akasha-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@earendil-works/akasha-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@earendil-works/akasha-tui";

akasha.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while akasha is streaming a response.
...
// Restore akasha's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@earendil-works/akasha-tui";
