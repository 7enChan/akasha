> akasha can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to akasha's agent capabilities. Use it to embed akasha in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @earendil-works/akasha-coding-agent
```
...
```typescript
import { createAgentSession } from "@earendil-works/akasha-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `akasha.sendMessage()`.
...
The `Agent` class (from `@earendil-works/akasha-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.akasha/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.akasha/extensions/`)
- Project skills:
  - `.akasha/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.akasha/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.akasha/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@earendil-works/akasha-ai";
import { AuthStorage, ModelRegistry } from "@earendil-works/akasha-coding-agent";
...
// Default: uses ~/.akasha/agent/auth.json and ~/.akasha/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/akasha-coding-agent";
...
- If you omit `tools`, akasha automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@earendil-works/akasha-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `akasha.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `akasha.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.akasha/agent/extensions/`, `.akasha/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `akasha.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/akasha-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/akasha-coding-agent";
...
1. Global: `~/.akasha/agent/settings.json`
2. Project: `<cwd>/.akasha/settings.json`
