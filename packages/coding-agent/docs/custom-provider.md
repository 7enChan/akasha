...
Extensions can register custom model providers via `akasha.registerProvider()`. This enables:
...
export default function (akasha: ExtensionAPI) {
  ...
  akasha.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. akasha waits for the factory before startup continues, so the provider is available during interactive startup and to `akasha --list-models`.
...
akasha.registerProvider("openai", {
  headers: {
...
akasha.registerProvider("google", {
...
```typescript
akasha.registerProvider("my-llm", {
...
Use `akasha.unregisterProvider(name)` to remove a provider that was previously registered via `akasha.registerProvider(name, ...)`:
...
// Later, remove it
akasha.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map akasha levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
akasha.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@earendil-works/akasha-ai";
...
Credentials are persisted in `~/.akasha/agent/auth.json`:
...
- [anthropic.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/7enChan/akasha/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/7enChan/akasha/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps akasha thinking levels to provider/model-specific values; null marks a level unsupported. */
