> akasha can create themes. Ask it to build one for your setup.
...
Akasha loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.akasha/agent/themes/*.json`
- Project: `.akasha/themes/*.json`
- Packages: `themes/` directories or `akasha.themes` entries in `package.json`
...
On first run, akasha detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.akasha/agent/themes
vim ~/.akasha/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/akasha/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, akasha reloads it automatically for immediate visual feedback.
...
Akasha uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, akasha falls back to the nearest approximation.
