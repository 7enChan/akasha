...
function isOfflineModeEnabled(): boolean {
	const value = process.env.AKASHA_OFFLINE;
...
type SkillDiscoveryMode = "akasha" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "akasha" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readAkashaManifestFile(packageJsonPath: string): AkashaManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { akasha?: AkashaManifest };
		return pkg.akasha ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readAkashaManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "akasha");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "akasha-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "akasha-extensions", prefix, hash, suffix ?? "");
	}
	...
	private collectPackageResources(
		...
		const manifest = this.readAkashaManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof AkashaManifest];
	...
	private readAkashaManifest(packageRoot: string): AkashaManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		// Project extensions from .akasha/
		addResources(
			"extensions",
		...
		// Project skills from .akasha/
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "akasha"),
			projectMetadata,
		...
		// User extensions from ~/.akasha/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.akasha/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "akasha"),
			userMetadata,
