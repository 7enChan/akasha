 ...
 *
 * Usage: akasha --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/akasha-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/akasha-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/akasha-tui";
...
export default function (akasha: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like akasha-doom would need)
	akasha.registerCommand("overlay-animation", {
	...
	akasha.registerCommand("overlay-anchors", {
	...
	akasha.registerCommand("overlay-margins", {
	...
	akasha.registerCommand("overlay-stack", {
	...
	akasha.registerCommand("overlay-overflow", {
	...
	akasha.registerCommand("overlay-edge", {
	...
	akasha.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	akasha.registerCommand("overlay-maxheight", {
	...
	akasha.registerCommand("overlay-sidepanel", {
	...
	akasha.registerCommand("overlay-toggle", {
	...
	akasha.registerCommand("overlay-passive", {
	...
	akasha.registerCommand("overlay-focus", {
	...
	akasha.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/akasha/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/akasha\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like akasha-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (akasha-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
