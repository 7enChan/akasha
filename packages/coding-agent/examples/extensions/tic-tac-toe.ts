...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@earendil-works/akasha-coding-agent";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@earendil-works/akasha-tui";
...
export default function (akasha: ExtensionAPI) {
	akasha.on("session_start", async (_event, ctx) => reconstructState(ctx));
	akasha.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	akasha.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	akasha.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	akasha.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			akasha.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						akasha.appendEntry(SAVE_TYPE, getBoardDetails());
