 ...
 * - ~/.akasha/agent/presets.json (global)
 * - <cwd>/.akasha/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `akasha --preset plan` - start with plan preset
...
import type { Api, Model } from "@earendil-works/akasha-ai";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/akasha-coding-agent";
import { DynamicBorder, getAgentDir } from "@earendil-works/akasha-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@earendil-works/akasha-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".akasha", "presets.json");
...
export default function presetExtension(akasha: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: akasha.getThinkingLevel(),
				tools: akasha.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			akasha.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = akasha.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				akasha.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.akasha/agent/presets.json or .akasha/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await akasha.setModel(originalState.model);
				}
				akasha.setThinkingLevel(originalState.thinkingLevel);
				akasha.setActiveTools(originalState.tools);
			} else {
				akasha.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	akasha.registerShortcut(Key.ctrlShift("u"), {
	...
	akasha.on("before_agent_start", async (event) => {
	...
	akasha.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = akasha.getFlag("preset");
	...
	akasha.on("turn_start", async () => {
		if (activePresetName) {
			akasha.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
