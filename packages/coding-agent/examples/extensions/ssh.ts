 ...
 *
 * Usage:
 *   akasha -e ./ssh.ts --ssh user@host
 *   akasha -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (akasha: ExtensionAPI) {
	akasha.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	akasha.on("session_start", async (_event, ctx) => {
		...
		const arg = akasha.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
