...
async function resolveGitHubRepo(akasha: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await akasha.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (akasha: ExtensionAPI): void {
	akasha.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(akasha, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await akasha.exec(
					"gh",
					[
						"issue",
						"list",
