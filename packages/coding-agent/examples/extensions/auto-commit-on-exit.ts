...
export default function (akasha: ExtensionAPI) {
	akasha.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await akasha.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[akasha] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await akasha.exec("git", ["add", "-A"]);
		const { code: commitCode } = await akasha.exec("git", ["commit", "-m", commitMessage]);
