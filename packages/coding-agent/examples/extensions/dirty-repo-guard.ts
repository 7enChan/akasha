...
async function checkDirtyRepo(
	...
	const { stdout, code } = await akasha.exec("git", ["status", "--porcelain"]);
...
export default function (akasha: ExtensionAPI) {
	akasha.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(akasha, ctx, action);
	});

	akasha.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(akasha, ctx, "fork");
	});
}
