...
Example extensions for akasha-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
akasha --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.akasha/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `akasha.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs akasha theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `akasha.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using akasha-ai's built-in Anthropic/OpenAI streaming via proxy |
