...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@earendil-works/akasha-tui";
...
export default function (akasha: ExtensionAPI) {
	akasha.registerCommand("invaders", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						akasha.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
