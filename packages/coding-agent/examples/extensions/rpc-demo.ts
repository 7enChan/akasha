...
export default function (akasha: ExtensionAPI) {
	...
	akasha.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "akasha RPC Demo (new session)" : "akasha RPC Demo");
	...
	akasha.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	akasha.on("turn_end", async (_event, ctx) => {
	...
	akasha.registerCommand("rpc-input", {
	...
	akasha.registerCommand("rpc-editor", {
	...
	akasha.registerCommand("rpc-prefill", {
