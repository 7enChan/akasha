 ...
 *
 * Demonstrates akasha.sendUserMessage() for sending user messages from extensions.
 * Unlike akasha.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (akasha: ExtensionAPI) {
	...
	akasha.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			akasha.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	akasha.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				akasha.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	akasha.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				akasha.sendUserMessage(args, { deliverAs: "followUp" });
	...
	akasha.registerCommand("askwith", {
