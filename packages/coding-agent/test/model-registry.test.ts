...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@earendil-works/akasha-ai";
import { getApiProvider } from "@earendil-works/akasha-ai";
import { getOAuthProvider } from "@earendil-works/akasha-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `akasha-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
