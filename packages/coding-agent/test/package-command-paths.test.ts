...
describe("package commands", () => {
	...
	let originalAkashaPackageDir: string | undefined;
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `akasha-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalAkashaPackageDir = process.env.AKASHA_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("akasha install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "akasha --help" or "akasha install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: akasha install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "akasha-coding-agent");
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/akasha" ? "@newer-scope/akasha" : "@new-scope/akasha";
		vi.stubGlobal(
			"fetch",
...
`,
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain(`Updated akasha`);
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:akasha-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "akasha-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:akasha-formatter?");
			expect(stdout).not.toContain("Updated akasha-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:akasha-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
