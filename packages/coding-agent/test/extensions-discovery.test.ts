...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "akasha-ext-test-"));
	...
	const extensionCode = `
		export default function(akasha) {
			akasha.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json akasha field", async () => {
	...
	it("package.json with akasha field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without akasha field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ akasha: { extensions: ["./entry.ts"] } }));
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(akasha) {
				akasha.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(akasha) {
				akasha.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(akasha) {
				akasha.on("agent_start", async () => {});
				akasha.on("tool_call", async (event) => undefined);
				akasha.on("agent_end", async () => {});
			}
		`;
	...
	it("loads extension with shortcuts", async () => {
		const extCode = `
			export default function(akasha) {
				akasha.registerShortcut("ctrl+t", {
