...
function parseArgs(argv: string[]): Args {
	...
	let sessionPath = resolve(join(tmpdir(), `akasha-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`));
...
function buildPrompt(turn: number): string {
	...
	const lines: string[] = [];
	lines.push("This is an automated prompt-cache tool-loop test executed through the akasha coding-agent SDK.");
	...
	for (let i = 1; i <= 180; i++) {
		lines.push(
			`Turn ${turn} synthetic record ${String(i).padStart(3, "0")}: alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron akasha rho sigma tau upsilon phi chi psi omega.`,
		);
	}
	lines.push("");
