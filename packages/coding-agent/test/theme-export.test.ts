...
describe("getThemeExportColors", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "akasha-theme-export-"));
		previousAgentDir = process.env.AKASHA_CODING_AGENT_DIR;
		process.env.AKASHA_CODING_AGENT_DIR = join(tempRoot, "agent");
		mkdirSync(join(process.env.AKASHA_CODING_AGENT_DIR, "themes"), { recursive: true });
	});
	...
	it("resolves export variable references using the same syntax as colors", () => {
		...
		writeFileSync(
			join(process.env.AKASHA_CODING_AGENT_DIR!, "themes", "custom-export-vars.json"),
	...
	it("resolves recursive vars and converts 256-color export values to hex", () => {
		...
		writeFileSync(
			join(process.env.AKASHA_CODING_AGENT_DIR!, "themes", "custom-export-recursive.json"),
			JSON.stringify(customTheme, null, 2),
		);
