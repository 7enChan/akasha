...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "akasha-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "akasha-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "akasha-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ akasha: { extensions: ["./akasha-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "akasha-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "akasha-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".akasha", "git", "github.com", "test", "extension");
