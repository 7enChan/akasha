...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@earendil-works/akasha-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.akasha/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".akasha", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.akasha/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.akasha/agent/auth.json
 */
...
/** Path to the real akasha agent config directory */
export const AKASHA_AGENT_DIR = join(homedir(), ".akasha", "agent");

/**
 * Get an AuthStorage instance backed by ~/.akasha/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `akasha-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
