 ...
 * 1. Akasha starts with settings.json containing packages: ["npm:some-pkg"]
...
describe("SettingsManager - External Edit Preservation", () => {
	...
	it("should preserve file changes to packages array when changing unrelated setting", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				theme: "dark",
				packages: ["npm:akasha-mcp-adapter"],
			}),
		);

		// Akasha starts up, loads settings into memory
		const manager = SettingsManager.create(projectDir, agentDir);

		// At this point, globalSettings.packages = ["npm:akasha-mcp-adapter"]
		expect(manager.getPackages()).toEqual(["npm:akasha-mcp-adapter"]);
