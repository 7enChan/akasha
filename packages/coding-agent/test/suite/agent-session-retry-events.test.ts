...
import { fauxAssistantMessage, fauxThinking, fauxToolCall } from "@earendil-works/akasha-ai";
...
describe("AgentSession retry and event characterization", () => {
	...
	it("prompt waits for retry completion even when assistant message_end handling is delayed", async () => {
		const harness = await createHarness({
			...
			extensionFactories: [
				(akasha) => {
					akasha.on("message_end", async (event) => {
	...
	it("emits extension events before public event subscribers", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(akasha) => {
					akasha.on("message_start", async (event) => {
