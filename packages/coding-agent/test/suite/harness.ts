...
import type { AgentMessage, AgentTool } from "@earendil-works/akasha-agent-core";
import { Agent } from "@earendil-works/akasha-agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@earendil-works/akasha-ai";
import { registerFauxProvider } from "@earendil-works/akasha-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `akasha-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
