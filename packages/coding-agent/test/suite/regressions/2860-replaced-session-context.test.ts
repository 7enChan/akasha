...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `akasha-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale akasha/ctx", async () => {
					...
					handler: async (_args, ctx) => {
						...
						await ctx.newSession({
							...
							withSession: async (replacedCtx) => {
								...
								try {
									oldAkashaApi?.sendUserMessage("stale message");
								} catch {
									staleAkashaApiThrows = true;
								}
		...
		expect(staleAkashaApiThrows).toBe(true);
	...
	it("supports withSession for fork", async () => {
		const { runtime } = await createRuntimeForTest(
			(akasha) => {
				akasha.registerCommand("fork-it", {
					description: "fork-it",
					handler: async (_args, ctx) => {
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(akasha) => {
				akasha.registerCommand("switch-it", {
