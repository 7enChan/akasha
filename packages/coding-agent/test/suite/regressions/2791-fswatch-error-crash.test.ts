...
/**
 * Regression test for https://github.com/7enChan/akasha/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "akasha-2791-"));
...
process.env.AKASHA_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync("npx", ["tsx", scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, AKASHA_CODING_AGENT_DIR: agentDir },
