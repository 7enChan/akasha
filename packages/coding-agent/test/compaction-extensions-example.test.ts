...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (akasha: ExtensionAPI) => {
			akasha.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (akasha: ExtensionAPI) => {
			akasha.on("session_compact", async (event: SessionCompactEvent) => {
