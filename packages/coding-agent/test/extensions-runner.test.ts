...
describe("ExtensionRunner", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "akasha-runner-test-"));
	...
	describe("shortcut conflicts", () => {
		it("warns when extension shortcut conflicts with built-in", async () => {
			const extCode = `
				export default function(akasha) {
					akasha.registerShortcut("ctrl+c", {
		...
		it("allows a shortcut when the reserved set no longer contains the default key", async () => {
			const extCode = `
				export default function(akasha) {
					akasha.registerShortcut("ctrl+p", {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(akasha) {
					akasha.registerShortcut("${pasteImageKey}", {
		...
		it("blocks shortcuts when reserved action has multiple keys", async () => {
			const extCode = `
				export default function(akasha) {
					akasha.registerShortcut("ctrl+y", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(akasha) {
					akasha.registerShortcut("ctrl+shift+x", {
	...
	describe("command collection", () => {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			const cmdCode = (description: string) => `
				export default function(akasha) {
					akasha.registerCommand("shared-cmd", {
	...
	describe("message renderers", () => {
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(akasha) {
					akasha.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
	...
	describe("flags", () => {
		...
		it("keeps first flag when two extensions register the same name", async () => {
			const first = `
				export default function(akasha) {
					akasha.registerFlag("shared-flag", {
						description: "first",
						type: "boolean",
						default: true,
					});
	...
	describe("before_agent_start", () => {
		it("keeps ctx.getSystemPrompt() in sync with chained system prompt updates", async () => {
			const extCode1 = `
				export default function(akasha) {
					akasha.on("before_agent_start", async (_event, ctx) => {
						return {
	...
	describe("tool_result chaining", () => {
		it("chains content modifications across handlers", async () => {
			const extCode1 = `
				export default function(akasha) {
					akasha.on("tool_result", async (event) => {
						return {
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(akasha) {
					akasha.on("tool_call", async () => undefined);
				}
			`;
