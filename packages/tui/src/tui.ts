...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.AKASHA_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.AKASHA_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.AKASHA_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".akasha", "agent", "akasha-debug.log");
		...
		// Configurable via setClearOnShrink() or AKASHA_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".akasha", "agent", "akasha-crash.log");
				const crashData = [
		...
		if (process.env.AKASHA_TUI_DEBUG === "1") {
