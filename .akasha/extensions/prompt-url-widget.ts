import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@earendil-works/akasha-coding-agent";
import { Container, Text } from "@earendil-works/akasha-tui";
...
async function fetchGhMetadata(
	...
	try {
		const result = await akasha.exec("gh", args);
...
export default function promptUrlWidgetExtension(akasha: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = akasha.getSessionName()?.trim();
		if (!currentName) {
			akasha.setSessionName(desiredName);
			return;
		}
	...
	akasha.on("before_agent_start", async (event, ctx) => {
		...
		void fetchGhMetadata(akasha, match.kind, match.url).then((meta) => {
	...
	akasha.on("session_switch", async (_event, ctx) => {
